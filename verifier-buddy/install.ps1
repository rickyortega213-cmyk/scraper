...
# "Continue" on purpose: under "Stop", git writing a harmless line to stderr
# would abort the script in Windows PowerShell 5.1. Exit codes are checked instead.
...
foreach ($candidate in @(@("py", "-3"), @("python"), @("python3"))) {
    ...
    if (Get-Command $exe -ErrorAction SilentlyContinue) {
        $ver = $null
        try { $ver = & $exe @($extra + @("-c", "import sys; print(sys.version_info >= (3, 9))")) 2>$null } catch { $ver = $null }
        if ("$ver".Trim() -eq "True") { $py = (@($exe) + $extra) -join " "; break }
    }
}
if (-not $py) {
...
if (Test-Path (Join-Path $RepoDir "verifier-buddy\verifier")) {
    ...
    if (-not $Branch) { Write-Host "  (could not read the default branch from GitHub; keeping the current one)" }
    ...
    if ($LASTEXITCODE -ne 0 -or -not (Test-Path (Join-Path $RepoDir "verifier-buddy\verifier"))) {
        throw "git clone failed; check your internet connection and re-run."
    }
}
...
if "%VERIFIER_NO_UPDATE%"=="1" goto run
where git >nul 2>&1 || goto run
...
if (-not (($userPath -split ";") -contains $BinDir)) {
    $newPath = if ($userPath.Trim()) { $userPath.TrimEnd(";") + ";" + $BinDir } else { $BinDir }
